...
- Set safe defaults in app/chrome-extension/entrypoints/
  shared/utils/tool-groups.ts: - observe=true, navigate=true, interact=false,
...
- Replace group-based phrasing with a concise allowlist:
  - “Allowed browser tools: … (IDs). Do not use other
    browser tools.”
- Implement this in:
  - app/chrome-extension/entrypoints/sidepanel/composables/
    useOpenClawChat.ts
  - backed by a new helper in app/chrome-extension/
    entrypoints/shared/utils/tool-groups.ts: - getEffectiveEnabledToolIds(state, individualState):
    string[]
