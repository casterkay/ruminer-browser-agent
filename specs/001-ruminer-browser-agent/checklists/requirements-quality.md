...
- [ ] CHK006 Are the tool-call inline rendering requirements complete (what statuses exist, what fields must be displayed, truncation rules for large tool payloads)? [Completeness, Spec US1]
  - US1 acceptance #4 specifies statuses (pending/success/error) and rendering (expandable cards). **Gap**: no truncation rules for large payloads, no minimum set of required display fields per card.
...
- [x] CHK009 Are cursor/checkpoint keys and cursor advancement rules specified in requirements (not only in blueprint)? [Gap, Spec FR-021, Edge Cases]
  - data-model.md §4.4: cursor convention `$cursor.{workflow_id}.last_seen_time` and `$cursor.{workflow_id}.last_seen_conversation_id`. Advancement rule: "Cursor advances only after ledger commit with status in {ingested, skipped}" (data-model.md §3). Edge case and US4 acceptance #5 both state cursor preservation on stop/failure. Session 2026-02-27 clarification: "cursor stays at the last committed ledger entry."
...
- [ ] CHK038 Are potentially ambiguous UX terms in the spec quantified ("visible progress", "clear message", "suggested next steps", "action required")? [Ambiguity, Spec US1–US4]
  - "visible progress": US2#2 partially quantifies as "current step, items processed." "action required": FR-023 specifies "please log in" for auth failures. **Remaining gaps**: "clear message" (FR-033, FR-034, US2#1) and "suggested next steps" (US2#1) are not quantified or enumerated.
- [ ] CHK039 Are there any conflicts between user stories and functional requirements (e.g., Memory tab delete semantics vs EMOS API support) explicitly resolved in the requirements? [Conflict, Spec FR-010]
  - FR-010 requires "delete items" from Memory tab. contracts/emos.md §4 specifies `POST /memories` (add) and `POST /memories/search` (search) but **no delete endpoint**. Whether EMOS supports deletion is not confirmed in the spec or contracts.
