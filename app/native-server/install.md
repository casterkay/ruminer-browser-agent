...
└─ postinstall.js
   ...
   └─ 如果失败 ➜ 提示用户运行 chrome-mcp-server register --system
      └─ 需要手动使用管理员权限运行
```
...
```bash
# macOS/Linux
sudo chrome-mcp-server register --system
...
```
sudo chrome-mcp-server register
```
...
```
注册流程
├─ 用户级别注册 (chrome-mcp-server register)
│  ├─ 获取用户级别清单路径
│  ├─ 创建用户目录
│  ├─ 生成清单内容
│  ├─ 写入清单文件
│  └─ Windows平台：创建用户级注册表项
│
└─ 系统级别注册 (chrome-mcp-server register --system)
   ├─ 检查是否有管理员权限
   │  ├─ 有权限 → 直接创建系统目录和写入清单
   │  └─ 无权限 → 提示用户使用管理员权限运行
   └─ Windows平台：创建系统级注册表项
...
3. 如果没有管理员权限：
   - 提示用户使用管理员权限重新运行命令
   - macOS/Linux: `sudo chrome-mcp-server register --system`
   - Windows: 以管理员身份运行命令提示符
...
│  │  └─ 运行 chrome-mcp-server fix-permissions
│  │
│  └─ 尝试 chrome-mcp-server register --system
│
├─ 路径问题
│  ├─ 检查Node.js安装 (node -v)
...
3. **修复执行权限问题**
   ...
   **解决方案**：
   ...
   ```bash
   chrome-mcp-server fix-permissions
   ```
   ...
   ```bash
   # 查找安装路径
   npm list -g chrome-mcp-server
   # 或者对于 pnpm
   pnpm list -g chrome-mcp-server

   # 设置执行权限（替换为实际路径）
   chmod +x /path/to/node_modules/chrome-mcp-server/run_host.sh
   chmod +x /path/to/node_modules/chrome-mcp-server/index.js
   chmod +x /path/to/node_modules/chrome-mcp-server/cli.js
   ```
   ...
   **解决方案**：
   ...
   ```bash
   # 卸载
   npm uninstall -g chrome-mcp-server
   # 或 pnpm uninstall -g chrome-mcp-server

   # 重新安装
   npm install -g chrome-mcp-server
   # 或 pnpm install -g chrome-mcp-server
...
5. 尝试使用系统级别安装
   - 使用 `chrome-mcp-server register --system` 命令
   - 或直接使用管理员权限运行
