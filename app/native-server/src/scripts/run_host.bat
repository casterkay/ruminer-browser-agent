...
REM Windows: %LOCALAPPDATA%\chrome-mcp-server\logs
set "LOG_DIR=%LOCALAPPDATA%\chrome-mcp-server\logs"
...
if not defined NODE_EXEC (
    ...
    echo To fix: Set CHROME_MCP_NODE_PATH environment variable or run 'chrome-mcp-server doctor --fix' >> "%WRAPPER_LOG%"
    exit /B 1
)
