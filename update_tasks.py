...
with open(
    "/Users/tcai/Projects/Ruminer/ruminer-browser-agent/specs/001-ruminer-browser-agent/tasks.md",
    "r",
) as f:
    lines = f.readlines()
...
for line in lines:
    ...
    if line.startswith("- [ ] T") and "[FR-" not in line:
        ...
        if match:
            ...
            if "US5" in tag:
                fr = "FR-019"
            if "tool group" in rest.lower() or "gate" in rest.lower():
                fr = "FR-025"
            if "ledger" in rest.lower():
                fr = "FR-018"
            if "hash" in rest.lower() or "item_key" in rest.lower():
                fr = "FR-017"
            if (
                "gateway" in rest.lower()
                or "node" in rest.lower()
                or "ws" in rest.lower()
            ):
                fr = "FR-013"
